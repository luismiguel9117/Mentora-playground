...
export default async function handler(req, res) {
  ...
  try {
    ...
    // To prevent Vercel serverless timeouts (10s limit), translate a maximum of 50 cues.
    const maxCues = 50;
    const cuesToTranslate = parsedCues.slice(0, maxCues);
    const isSpanish = sourceLang === 'es';
    const isEnglish = sourceLang === 'en';

    // Parallel chunked translation
    const chunkSize = 5;
    for (let i = 0; i < cuesToTranslate.length; i += chunkSize) {
      const chunk = cuesToTranslate.slice(i, i + chunkSize);
      await Promise.all(chunk.map(async (cue) => {
      ...
      // Wait a tiny bit between chunks to prevent MyMemory API rate limits
      await new Promise(r => setTimeout(r, 60));
    }
